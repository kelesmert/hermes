...
Üretim verilerinin doğası gereği, farklı kaynaklardan gelen kayıtların aynı zaman ekseninde tutarlı biçimde ele alınması önemli bir zorluk alanıdır. Özellikle vardiya pencereleri, veri kaynakları ve kayıtların gecikmeli/eksik gelmesi gibi durumlar, raporların güvenilirliğini doğrudan etkileyebilir. Bu nedenle raporlama katmanında, “hangi pencere ve hangi kaynakla” hesap yapıldığının açık biçimde görünür kılınması kritik hale gelmiştir.
...
İlerleyen aşamada sistemin saha kullanımına daha yakın hale getirilmesi için gerçek veri toplama katmanının güçlendirilmesi önerilmektedir. Bu kapsamda farklı veri kaynaklarının (sensör, PLC, dış sistem) entegrasyonu için standartlaştırılmış bir veri alım katmanı ve mesajlaşma/queue tabanlı mimari seçenekleri değerlendirilebilir. Böyle bir genişleme, canlı izleme ve raporlamanın ölçeklenebilirliğini artıracaktır.
