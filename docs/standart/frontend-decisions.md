...
- **Dashboard Operasyon Görünümü (`/dashboard`):** Board domain endpoint'i (`/api/board/operations`) üzerinden veri alır. React Query `refetchInterval` ile polling yapılır; KPI kartları, durum dağılımı grafiği ve duruş listesi MUI Card + Grid ile render edilir. UI, `paused` job'ları (job listesi `/production/job-orders` üzerinden çekilir) `idle` olarak gösterir ve “Job duraklatıldı” notu düşer; job-status eslestirmesinde machine code fallback kullanilir.
...
- **Downtime Sayfası (`/downtimes`):** Açık duruşlar, planlı duruş kuralları ve run geçmişi ile geçmiş duruş filtrelerini içerir. Operatör job aktifken manuel plansız duruş başlatabilir (reason zorunlu). Reason sınıflandırma akışı 5 dk düzeltme penceresi ve split semantiğine göre çalışır; 5 dk’dan uzun telemetry plansız duruşlar “Onay Bekliyor” olarak işaretlenir ve geçmiş listeden onaylanır.
