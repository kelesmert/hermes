...
- **MachineTelemetry sistemi (zorunlu):** Her makine için 0/1 sinyal değeri, timestamp ve metrikler (sıcaklık, tork, enerji) `machine_telemetry` koleksiyonunda saklanır. Her kayıt `source` alanı ile hangi üreticiden geldiğini taşır (`data-gen`, `shift-sim`). Simülasyon kaynakları:
  ...
  - `scripts/shift-simulator.js`: 07:00–18:00 vardiyası için deterministik, hızlandırılmış telemetry üretir ve kayıtları `simulationRunId` ile etiketler (`npm run shift:sim`). Sinyal 1 yalnızca makinede `in_progress` job varken üretilir; koşu bitince sistem `shift_end` uygular (job `paused`, makine `idle`, açık event’ler vardiya bitişinde kapanır). `data-gen` ile aynı anda çalıştırılmaz. Telemetry kayıtlarında ilgili job için `jobOrder` alanı yazılır. Ayrica 14:30–16:30 araliginda tek bir plansiz durus blogu uretilir (60–120 dk). Shift bitiminde `jobOrder` bos, `signalValue=0` final telemetry yazilir (`intervalMs=1`).
- **Simulation Clock (shift-sim) (zorunlu):** Shift-sim deterministik bir zaman çizgisi üretmek için DB’de kalıcı state tutar. `SHIFT_SIM_EPOCH_DATE` başlangıç günüdür; `cursorAt < shiftEndAt` ise aynı gün içinde aynı `simulationRunId` ile devam edilir, shift bittiğinde sonraki start ertesi gün 07:00’den yeni run ile başlar. State ile DB çelişirse DB’deki en büyük `source=shift-sim` telemetry timestamp’i baz alınır ve state düzeltilir (duplicate üretmek yerine).
- **Production simülasyonu (zorunlu):** Üretim verisi yalnızca telemetry sinyali 1 iken yazılmalıdır. `backend/scripts/job-simulator.js` aktif JobOrder kayıtlarını okuyup telemetry timestamp’lerine göre ideal çevrim süresinden üretim miktarı hesaplar ve good/defect eventleri oluşturur. Telemetry yoksa job-sim üretim yapmaz; telemetry kaynağı `JOB_SIM_TELEMETRY_SOURCE` ile explicit seçilir (varsayılan `shift-sim`). Job-sim telemetry’yi `jobOrder` alanına göre filtreleyerek yalnızca ilgili job’a ait kayıtları işler.
  - Job start/pause/resume/complete event’leri kaynağa göre zaman ekseni kullanır: `shift-sim` için sim-clock, `data-gen` için wall-clock (`JOB_TIME_SOURCE`).
...
- **Simülasyon kontrolü (zorunlu):** UI üzerinden simülasyon çalıştırmak için `/api/simulations` endpoint’leri kullanılır (başlat/durdur/log). Endpoint’ler `production.manage` ile korunur; prod ortamında `ENABLE_SIMULATION_CONTROL=true` değilse kontrol kapalıdır. Log buffer sınırı `SIMULATION_LOG_MAX_LINES`, stop timeout `SIMULATION_STOP_TIMEOUT_MS` env’leriyle yönetilir. Shift-sim reset için `POST /api/simulations/shift-sim/reset` kullanılır; sadece shift-sim telemetry ve sim kaynaklı event/state’leri temizler ve sim çalışırken reset’e izin verilmez.
- **OEE Domain:** `domains/oee` altında `OeeMachineState` modeli ve processor job bulunur. OEE Processor Job (`src/jobs/oee-processor-job.js`) telemetry verilerini batch olarak işler ve belirli süre (threshold) 0 sinyali algılandığında, makine `in_progress` bir iş emri yürütüyorsa plansız duruş timing’ini tespit eder. Event yazımı doğrudan `MachineEvent.create` ile yapılmaz, downtime domain orchestrator üzerinden orkestre edilir; job yoksa status `idle` olarak korunur ve job `paused` iken telemetry 0 serileri plansız duruş açmaz. `OeeMachineState` modeli her makine için son sinyal değeri, aktif duruş event'i ve sıfır serisi başlangıç zamanını tutar.
- **OEE telemetry source izolasyonu (zorunlu):** OEE processor telemetry sorguları `source` filtresi ile yapılır. Tez demosunda varsayılan olarak yalnızca `source=shift-sim` işlenir (`OEE_PROCESSOR_TELEMETRY_SOURCE=shift-sim`); “canlı izleme” için data-gen çalışsa bile OEE cursor’u bozulmaz.
...
- **OEE processor kapasite ayarı (önerilen):** Telemetry işleme gecikmesi (processedAt geride) veya dashboard duruş listesi gecikmesi görülürse önce `oee-rules.json` içindeki `aggregation.batchSize` artırılır. `pollIntervalMs` düşürmek overlap riskini artırabileceği için ikinci tercihtir.
