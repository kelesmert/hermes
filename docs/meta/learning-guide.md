...
- `backend/src/domains/oee/services/oee-processor.js` telemetry verilerini JSON konfigine göre işler; `backend/src/jobs/oee-processor-job.js` belirli aralıklarla bu servisi tetikler ve duruş eventlerini otomatik oluşturur/kapatır.
- `backend/src/domains/board/services/board-service.js` OEE sonuçları + telemetry ortalamalarını birleştirerek `/api/board/metrics` endpoint’ine veri sağlar (Dashboard izinli kullanıcılar 5-10 sn polling ile tüketir).
